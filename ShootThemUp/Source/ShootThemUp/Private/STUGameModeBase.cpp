...
void ASTUGameModeBase::CreateTeamsInfo()
{
	...
	for (auto It = GetWorld()->GetControllerIterator(); It; ++It)
	{
		...
		PlayerState->SetTeamData({TeamID, DetermineColorByTeamID(TeamID)});

		TArray<FString> ListNames{"Jack", "Ron", "Arthas", "Uther", "Killer", "Pit", "James"};
		PlayerState->SetPlayerName(Controller->IsPlayerController() ? "Player" : GetRandomName(ListNames));
...
{
	const bool IsPauseSet{Super::SetPause(PC, CanUnpauseDelegate)};
	if (IsPauseSet)
	{
...
FString ASTUGameModeBase::GetRandomName(const TArray<FString>& ListNames) const
{
	const auto Index{FMath::RandRange(0, ListNames.Num() - 1)};
